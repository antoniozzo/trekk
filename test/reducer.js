...
test('it reduces next progress', t => {
	const progress = nextProgress(0.555555, 0.888888, 1)

	t.is(progress, 0.888888)
})

test('it reduces next progress with linear interpolation', t => {
	const progress = nextProgress(0.555555, 0.888888, 0.5)

	t.is(progress, 0.7222215000000001)
})
...
test('it reduces next state', t => {
	...
	const reducer = makeReducer(fakeOptions)
